...
const input = `qFdBBvtHHfvRlfvsqldvqjPpQmnQmjnjjjTRTLGRNG
...
const getCodeByLetter = (text: string) => {
  const code = text.charCodeAt(0);
  if (code > 96) return code - 96;
  return code - 64 + 26;
};

const findCommon = (first: string, second: string, third: string) => {
  for (const firstLetter of first) {
    for (const secondLetter of second) {
      for (const thirdLetter of third) {
...
const getPrioritySum = (input: string) => {
  ...
  for (let i = 0; i < rucksacks.length; i += 3) {
    prioritySum += getCodeByLetter(
      findCommon(rucksacks[i], rucksacks[i + 1], rucksacks[i + 2])!
    );
  }
  return prioritySum;
};

console.log(getPrioritySum(sampleInput));
