...
class Directory {
  ...
  constructor(size: number, dirName: string, parent?: Directory) {
    this.size = size;
    this.dirName = dirName;
    if (parent) this.parent = parent;
  }

  addChild(subDir: Directory) {
...
const addToParentSize = (dir: Directory, size: number) => {
  if (dir.parent !== null) {
    const parentDir = dir.parent;
    parentDir.size += size;
    addToParentSize(parentDir, size);
  }
};

const dirTraversal = (input: string) => {
  const inputArray = convertInput(input);
  const parentDirectory = new Directory(0, '/');
  ...
  for (let i = 1; i < inputArray.length; i++) {
    ...
    if (currentLine.startsWith('$ ls')) {
      ...
      dirs.forEach((dir) =>
        directoryList.push(new Directory(0, dir, currentDirectory))
      );
      currentDirectory.size += size;
      addToParentSize(currentDirectory, size);
      if (endIndex === -1) break;
      i = endIndex;
    } else if (currentLine.startsWith('$ cd')) {
      const dirName = currentLine.split(' ')[2];
      if (dirName === '..') directoryDepth.pop();
      else {
        const subDir = currentDirectory.children.find(
          (dir) => dir.dirName === dirName
        );
        if (subDir === undefined) {
          const newSubDir = new Directory(0, dirName, currentDirectory);
          directoryList.push(newSubDir);
          directoryDepth.push(newSubDir);
        } else {
          directoryDepth.push(subDir);
        }
      }
    }
  }
